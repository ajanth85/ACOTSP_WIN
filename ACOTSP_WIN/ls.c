...
long int * generate_random_permutation( long int n, long int *seed )
/*    
...
*/
{
   ...
   if ((r = malloc(n * sizeof(long int))) == NULL) {
...
*/
{
	...
	for (l = 0 ; l < n; l++) {
	    ...
	    c1 = random_vector[l]; 
	    _DEBUG ( assert ( c1 < n && c1 >= 0); )
		if (ls_params->dlb_flag && dlb[c1] )
		    continue;
	    improve_node = FALSE;
	    pos_c1 = pos[c1];
	    s_c1 = tour[pos_c1+1];
...
void two_h_opt_first( long int *tour, struct problem *instance, int n, struct ls_parameters *ls_params, long int *seed) 
/*    
...
*/
{
	...
	if ( (pos = malloc(n * sizeof(long int))) == NULL) 
	{
		printf("Out of memory, exit");
		exit(1);
	}
	if ((dlb = malloc(n * sizeof(long int))) == NULL)
	{
		printf("Out of memory, exit");
		exit(1);
	}
	if ((h_tour = malloc(n * sizeof(long int))) == NULL)
	{
		printf("Out of memory, exit");
		exit(1);
	}
	if ((hh_tour = malloc(n * sizeof(long int))) == NULL)
	{
		printf("Out of memory, exit");
		exit(1);
	}

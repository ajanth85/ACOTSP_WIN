...
struct options {
    /* Set to 1 if option --threads (-j) has been specified*/
...
/* Argument to option --threads (-j).  */
const char* arg_threads;
...
{
   static const char* const optstr__threads = "threads";
    ...
    else if (*option == '-')
    {
      ...
      switch (*option)
      {
       ...
       case 't':
        ...
        else if (strncmp(option + 1, optstr__threads + 1, option_len - 1) == 0)
        {
            if (option_len <= 1)
                goto error_long_opt_ambiguous;
            if (argument != 0)
                options->arg_threads = argument;
            else if (++i < argc)
                options->arg_threads = argv[i];
            else
            {
                option = optstr__threads;
                goto error_missing_arg_long;
            }
            options->opt_threads = 1;
            break;
...
int parse_commandline (int argc, char *argv [], struct hyper_parameters *hyper, struct ant_hyper_parameters *ant_hyper, struct ls_parameters* ls_params)
{
    int i;
    const char *progname;
    ...
    if ( options.opt_time ) {
	hyper->max_time = atof(options.arg_time);
	fputs ("  -t  --time ", stdout);
	if (options.arg_time != NULL)
	    printf ("with argument \"%.3f\"\n", hyper->max_time);
	check_out_of_range( hyper->max_time, 0.0, 86400., "max_time (seconds)");
    } else {
	...
	hyper->max_tries = atol(options.arg_tries);
	fputs ("  -r  --tries ", stdout);
	if (options.arg_tries != NULL)
	    printf ("with argument \"%ld\"\n", hyper->max_tries);
	check_out_of_range( hyper->max_tries, 1, MAXIMUM_NO_TRIES, "max_tries (tries)");
    } else {
	hyper->max_tries = 10;
    ...
    if (options.opt_threads) {
        hyper->max_number_of_threads = atol(options.arg_threads);
        fputs("  -j  --threads ", stdout);
        if (options.opt_threads != NULL)
            printf("with argument \"%ld\"\n", hyper->max_number_of_threads);
        check_out_of_range(hyper->max_number_of_threads, 1, MAXIMUM_NO_THREADS, "max_tries (tries)");
    }
    else {
        hyper->max_number_of_threads = 11;
        fprintf(stderr, "\tNote: number or threads is set to default 1\n");
    }


    if ( options.opt_tours ) {
	hyper->max_tours = atol(options.arg_tours);
	fputs ("  -s  --tours ", stdout);
	if (options.arg_tries != NULL)
	    printf ("with argument \"%ld\"\n", hyper->max_tours);
	check_out_of_range( hyper->max_tours, 1, LONG_MAX, "max_tries (tries)");
    } else {
	hyper->max_tours = 100;
    ...
    {
    hyper->optimal = atol(options.arg_optimum);
    ...
    {
	strncpy( hyper->name_buf, options.arg_tsplibfile, strlen( options.arg_tsplibfile ) );
    ...
    if ( options.opt_ants ) {
        ant_hyper->n_ants = atol(options.arg_ants);
	fputs ("  -m  --ants ", stdout);
	if (options.arg_ants != NULL)
	    printf ("with argument \"%ld\"\n", ant_hyper->n_ants);
	check_out_of_range(ant_hyper->n_ants, 1, MAX_ANTS-1, "n_ants");
    } else {
        ant_hyper->n_ants = 25;
    ...
    if ( options.opt_nnants ) {
        ant_hyper->nn_ants = atol(options.arg_nnants);
	fputs ("  -m  --ants ", stdout);
	if (options.arg_ants != NULL)
	    printf ("with argument \"%ld\"\n", ant_hyper->nn_ants);
	check_out_of_range(ant_hyper->n_ants, 1, 100, "nn_ants");
    } else {
        ant_hyper->nn_ants = 20;
    ...
    if ( options.opt_alpha ) {
        ant_hyper->alpha = atof(options.arg_alpha);
	fputs ("  -a  --alpha ", stdout);
	if (options.arg_alpha != NULL)
	    printf ("with argument \"%f\"\n", ant_hyper->alpha);
	check_out_of_range(ant_hyper->alpha, 0., 100., "alpha");
    } else {
        ant_hyper->alpha = 1.0;
    ...
    if ( options.opt_beta ) {
        ant_hyper->beta = atof(options.arg_beta);
	fputs ("  -b  --beta ", stdout);
	if (options.arg_beta != NULL)
	    printf ("with argument \"%f\"\n", ant_hyper->beta);
	check_out_of_range(ant_hyper->beta, 0., 100., "beta");
    } else {
        ant_hyper->beta = 2.0;
    ...
    if ( options.opt_rho ) {
        ant_hyper->rho = atof(options.arg_rho);
	fputs ("  -e  --rho ", stdout);
	if (options.arg_rho != NULL)
	    printf ("with argument \"%f\"\n", ant_hyper->rho);
	check_out_of_range(ant_hyper->rho, 0., 1., "rho");
    } else {
        ant_hyper->rho = 0.5;
    ...
    if ( options.opt_q0 ) {
        ant_hyper->q_0 = atof(options.arg_q0);
	fputs ("  -q  --q0 ", stdout);
	if (options.arg_q0 != NULL)
	    printf ("with argument \"%f\"\n", ant_hyper->q_0);
	check_out_of_range(ant_hyper->q_0, 0., 1., "q0");
    } else {
        ant_hyper->q_0 = 0.0;
    ...
    if ( options.opt_elitistants ) {
        ant_hyper->elitist_ants = atol(options.arg_elitistants);
	fputs ("  -m  --ants ", stdout);
	if (options.arg_elitistants != NULL)
	    printf ("with argument \"%ld\"\n", ant_hyper->elitist_ants);
	check_out_of_range(ant_hyper->n_ants, 0, LONG_MAX, "elitistants");
    } else {
    ...
    if ( options.opt_rasranks ) {
        ant_hyper->ras_ranks = atol(options.arg_rasranks);
	fputs ("  -m  --ants ", stdout);
	if (options.arg_rasranks != NULL)
	    printf ("with argument \"%ld\"\n", ant_hyper->ras_ranks);
	check_out_of_range(ant_hyper->n_ants, 0, LONG_MAX, "rasranks");
    } else {
    ...
    if ( options.opt_nnls ) {
        ls_params->nn_ls = atol(options.arg_nnls);
	fputs ("  -k  --nnls ", stdout);
	if (options.arg_nnls != NULL)
	    printf ("with argument \"%ld\"\n", ls_params->nn_ls);
	check_out_of_range(ant_hyper->n_ants, 0, LONG_MAX, "nnls");
    } else {
        ls_params->nn_ls = 20;
    ...
    if ( options.opt_localsearch ) {
        ls_params->ls_flag = atol(options.arg_localsearch);
	fputs ("  -l  --localsearch ", stdout);
	if (options.arg_localsearch != NULL)
	    printf ("with argument \"%ld\"\n", ls_params->ls_flag);
	check_out_of_range(ant_hyper->n_ants, 0, LONG_MAX, "ls_flag");
    } else {
        ls_params->ls_flag = 3;
    ...
    if ( options.opt_dlb ) {
        ls_params->dlb_flag = atol(options.arg_dlb);
	fputs ("  -d  --dlb ", stdout);
	if (options.arg_dlb != NULL)
	    printf ("with argument \"%ld\"\n", ls_params->dlb_flag);
	check_out_of_range(ls_params->dlb_flag, 0, 1, "dlb_flag");
    } else {
        ls_params->dlb_flag = TRUE;
    ...
    if ( options.opt_as ) {
        ant_hyper->as_flag = TRUE;
        ant_hyper->eas_flag = FALSE; ant_hyper->ras_flag = FALSE; ant_hyper->mmas_flag = FALSE;
        ant_hyper->bwas_flag = FALSE; ant_hyper->acs_flag = FALSE;
...
/*  	fprintf(stderr,"\tNote: as_flag is set to default 0 (don't run Ant System)\n"); */
    ...
    if ( options.opt_eas ) {
        ant_hyper->eas_flag = TRUE;
        ant_hyper->as_flag = FALSE; ant_hyper->ras_flag = FALSE; ant_hyper->mmas_flag = FALSE;
        ant_hyper->bwas_flag = FALSE; ant_hyper->acs_flag = FALSE;
...
/*  	fprintf(stderr,"\tNote: eas_flag is set to default 0 (don't run Elitist Ant System)\n"); */
    ...
    if ( options.opt_ras ) {
        ant_hyper->ras_flag = TRUE;
        ant_hyper->as_flag = FALSE; ant_hyper->eas_flag = FALSE; ant_hyper->mmas_flag = FALSE;
        ant_hyper->bwas_flag = FALSE; ant_hyper->acs_flag = FALSE;
...
/*  	fprintf(stderr,"\tNote: ras_flag is set to default 0 (don't run rank-based Ant System)\n"); */
    ...
    if ( options.opt_mmas ) {
        ...
        ant_hyper->as_flag = FALSE; ant_hyper->eas_flag = FALSE; ant_hyper->ras_flag = FALSE;
        ant_hyper->bwas_flag = FALSE; ant_hyper->acs_flag = FALSE;
...
/*  	fprintf(stderr,"\tNote: mmas_flag is set to default 1 (run MAX-MIN Ant System!)\n"); */
    ...
    if ( options.opt_bwas ) {
        ant_hyper->bwas_flag = TRUE;
        ant_hyper->as_flag = FALSE; ant_hyper->eas_flag = FALSE; ant_hyper->mmas_flag = FALSE;
        ant_hyper->ras_flag = FALSE; ant_hyper->acs_flag = FALSE;
...
/*  	fprintf(stderr,"\tNote: bwas_flag is set to default 0 (don't run Best-Worst Ant System)\n"); */
    ...
    if ( options.opt_acs ) {
        ant_hyper->acs_flag = TRUE;
        ant_hyper->as_flag = FALSE; ant_hyper->eas_flag = FALSE; ant_hyper->mmas_flag = FALSE;
        ant_hyper->ras_flag = FALSE; ant_hyper->bwas_flag = FALSE;
